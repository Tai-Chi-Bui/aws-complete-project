...
```
docker run --rm -p 3000:3000 -it -e REACT_APP_BACKEND_URL="https://4567-${GITPOD_WORKSPACE_ID}.${GITPOD_WORKSPACE_CLUSTER_HOST}" frontend-react-
```


## Step 9: add ```docker-compose.yml``` to run ```backend-flask``` and ```frontend-react-js``` at the same time.
